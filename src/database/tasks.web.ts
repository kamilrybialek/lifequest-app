...
export const createTask = async (
  ...
  try {
    console.log('[tasks.web.ts] createTask called with userId:', userId, 'type:', typeof userId);
    const db = await getDatabase();
    const normalizedUserId = normalizeUserId(userId);
    console.log('[tasks.web.ts] Normalized userId:', normalizedUserId);
    ...
    console.log('[tasks.web.ts] Task inserted with ID:', taskId);

    // Add tags if provided
    if (taskData.tags && taskData.tags.length > 0) {
      console.log('[tasks.web.ts] Adding tags:', taskData.tags);
    ...
    console.log('[tasks.web.ts] Task created successfully:', taskId);
    return taskId;
  } catch (error) {
    console.error('[tasks.web.ts] Error in createTask:', error);
    throw error;
  }
};
