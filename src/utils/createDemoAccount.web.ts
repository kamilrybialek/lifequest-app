/**
 * DEMO ACCOUNT GENERATOR - WEB VERSION
 *
 ...
 * - Sample goals
 *
 * CRITICAL: Uses AsyncStorage versions (.web) to avoid SQLite imports
 */

import { createUser, updateUserOnboarding } from '../database/user.web';
import { createTask, initializeDefaultLists } from '../database/tasks.web';
import { createBudget, addExpense, addDebt } from '../database/finance.web';
...
export const createDemoAccount = async (): Promise<number> => {
  ...
  console.log('✅ [WEB] Demo account created with sample data');
  return userId;
};

export const getDemoUserId = async (): Promise<number | null> => {
  try {
    // CRITICAL: Use web version to avoid SQLite import
    const { getUserByEmail } = await import('../database/user.web');
    const user = await getUserByEmail(DEMO_EMAIL);
    return user ? user.id : null;
  } catch (error) {
    console.error('[WEB] Error getting demo user:', error);
    return null;
  }
};
